...
class GoogleSheetsIntegration:
    ...
    def update_profile_labels(self, profile_id: str, updated_labels: Dict[str, Any]) -> bool:
        """Update labels for an existing profile.
        
        Args:
            profile_id: ID of the profile to update
            updated_labels: Updated labels for the profile
            
        Returns:
            bool: True if update was successful, False otherwise
        """
        if not self.service:
            logger.error("Cannot update profile labels: Google Sheets service not initialized")
            return False
        ...
        try:
            ...
            if not data or len(data) <= 1:  # Only header or no data
                logger.warning(f"Cannot update labels for profile {profile_id}: No profiles found in sheet")
                return False
            
            # Find the row with matching profile ID
            profile_row_index = None
            for i, row in enumerate(data):
                if len(row) > 1 and row[1] == profile_id:
                    profile_row_index = i
                    break
            
            if profile_row_index is None:
                logger.warning(f"Cannot update labels for profile {profile_id}: Profile not found in sheet")
                return False
            
            # Update the JSON column with new labels
            row_number = profile_row_index + 1  # Adjust for 1-based indexing
            json_column = 24  # Column X (24th column) contains the full JSON
            
            # Update the cell
            self.service.spreadsheets().values().update(
                spreadsheetId=self.spreadsheet_id,
                range=f"{self.EXTRACTED_DATA_SHEET}!X{row_number}",
                valueInputOption='RAW',
                body={'values': [[json.dumps(updated_labels)]]}
            ).execute()
            
            # Also update individual label columns
            # Handle gender specially (it's a more complex structure)
            gender_labels = updated_labels.get('gender', [])
            gender_str = "Unknown"
            if gender_labels:
                # Find the gender with the highest maximum value
                max_gender = max(gender_labels, key=lambda x: x.get('value', [0, 0])[1])
            ...
            # Find primary ethnicity
            ethnicity_labels = updated_labels.get('ethnicity', [])
            primary_ethnicity = "Unknown"
            if ethnicity_labels:
                # Find the ethnicity with the highest maximum value
                max_ethnicity = max(ethnicity_labels, key=lambda x: x.get('value', [0, 0])[1])
            ...
            self.service.spreadsheets().values().update(
                spreadsheetId=self.spreadsheet_id,
                range=f"{self.EXTRACTED_DATA_SHEET}!L{row_number}",
                valueInputOption='RAW',
                body={'values': [[gender_str]]}
            ).execute()
            
            self.service.spreadsheets().values().update(
                spreadsheetId=self.spreadsheet_id,
                range=f"{self.EXTRACTED_DATA_SHEET}!P{row_number}",
                valueInputOption='RAW',
                body={'values': [[primary_ethnicity]]}
            ).execute()
            
            # Update numeric range attributes
            for label_name, column in label_columns.items():
                if label_name in updated_labels:
                    self.service.spreadsheets().values().update(
                        spreadsheetId=self.spreadsheet_id,
                        range=f"{self.EXTRACTED_DATA_SHEET}!{column}{row_number}",
                        valueInputOption='RAW',
                        body={'values': [[format_range(updated_labels[label_name])]]}
                    ).execute()
            
            logger.info(f"Successfully updated labels for profile {profile_id}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to update labels for profile {profile_id}: {e}")
            return False
